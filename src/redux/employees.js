...
export const Employees = (state = EMPLOYEES, action) => {
    ...
    //DELETING REVIEW
		case ActionTypes.REVIEW_REMOVED:
			const empCopy = state.find(
				(employee) => employee.name === action.payload.review.name
			);

			const newReviews = empCopy.reviews.filter(
		...
		case ActionTypes.REVIEW_CREATED:
			...
			const employeeCopy = state.find(
				(employee) => employee.name === action.payload.employeeName
			);
